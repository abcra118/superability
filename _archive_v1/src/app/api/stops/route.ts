...
export async function GET(req: NextRequest) {
  ...
  if (metroRes.status === 'fulfilled' && metroRes.value.data) {
    results.push(...metroRes.value.data.map((s: any) => ({ ...s, mode: (s.stop_name.toLowerCase().includes("bus") ? "bus" : (s.stop_name.toLowerCase().includes("tram") ? "tram" : "train")) })));
  }
  if (tramRes.status === 'fulfilled' && tramRes.value.data) {
   results.push(...tramRes.value.data.map((s: any) => ({ ...s, mode: 'tram' })));
  }
  if (busRes.status === 'fulfilled' && busRes.value.data) {
    results.push(...busRes.value.data.map((s: any) => ({ ...s, mode: 'bus' })));
  }
